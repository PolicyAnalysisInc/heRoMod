...
#' @param parallel Optional. Whether to use parallel evaluation of strategies. Defaults to `FALSE`.
#' @param cores Positive integer. Number of cores to use if `parallel = TRUE`. Defaults to 1.
#' @param disc_method Character. Discounting method, e.g., 'start', 'midpoint', 'end'. Defaults to 'start'.
#' @param state_groups Optional list or data frame defining groups of states. See details.
#' @param individual_level Logical. If `TRUE`, runs an individual-level simulation (microsimulation). Defaults to `FALSE`.
#' @param create_progress_reporter Function. A factory function that creates a progress reporter. (Advanced use)
#' @param progress_reporter An initialized progress reporter object. (Advanced use)
#'   
#' @return An object of class `run_model`. This object is a list containing:
#'   \item{run_model}{A data frame with the summary results for each strategy (total costs, effects, ICERs, etc.).}
#'   \item{eval_strategy_list}{A list where each element is an `eval_strategy` object, containing detailed cycle-by-cycle results for that strategy.}
#'   \item{uneval_strategy_list}{The original list of unevaluated strategy definitions (`uneval_model` objects).}
#'   \item{parameters}{The `uneval_parameters` object used.}
#'   \item{init, inflow, cycles, method, ce, central_strategy, etc.}{Other input parameters and derived settings.}
#' @export
#' 
...
#' @examples
#' \dontrun{
#' # This is a conceptual example. Actual definitions for define_strategy,
#' # define_state, define_transition are needed.
#' health_state <- define_state(cost = 100, ly = 1)
#' sick_state <- define_state(cost = 500, ly = 0.5)
#'
#' transition_matrix_strategy1 <- define_transition(
#'   C, 0.2, # from health_state to health_state, health_state to sick_state
#'   0.1, C  # from sick_state to health_state, sick_state to sick_state
#' )
#' strategy1 <- define_strategy(
#'   transition = transition_matrix_strategy1,
#'   health = health_state,
#'   sick = sick_state
#' )
#'
#' # Run the model for a single strategy
#' model_run_single <- run_model(
#'   strategy1,
#'   cycles = 20,
#'   init = c(health = 1000, sick = 0),
#'   cost = cost,
#'   effect = ly
#' )
#' print(model_run_single)
#' summary(model_run_single)
#' }
#'
#' @seealso [define_strategy()], [define_parameters()], [summary.run_model()], [plot.run_model()]
#'   
run_model <- function(...,
...
#' Standard Evaluation Version of run_model
#'
#' This is the standard evaluation (SE) counterpart to [run_model()].
#' It is intended primarily for programmatic use where strategy definitions and
#' parameters are already captured as lists of expressions or specific objects,
#' avoiding non-standard evaluation of `...` or direct formula inputs.
#' Most users should use [run_model()] directly.
#'
#' @export
#' @rdname run_model
