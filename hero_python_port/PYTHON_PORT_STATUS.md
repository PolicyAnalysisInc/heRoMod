...
*   **Phase 7: Documentation, Examples, and Refinement:** **IN PROGRESS**
    ...
    *   **Task 7.4: Comprehensive user guides (topical):** IN PROGRESS - Drafted guides: "Defining and Using Parameters", "Understanding and Using SimulationOutput Results", "Probabilistic Sensitivity Analysis (PSA) Workflow", "Deterministic Sensitivity Analysis (DSA) and Tornado Plots", "Model Calibration Workflow", "Value of Information (EVPI & EVPPI) Explained", "Value-Based Pricing (VBP) with `hero_py`", "Scenario Analysis Techniques", "Running Different Model Types: A Comparison (Markov vs. PSM)", "Customizing Plots in `hero_py`".
