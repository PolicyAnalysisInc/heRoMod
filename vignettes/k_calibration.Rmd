---
title: "Calibrating `heRomod` models"
date: "`r Sys.Date()`"
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Calibrating heRomod models}
...
In order to perform calibration, the user must provide:
  ...
  1. A heRomod object from `run_model()` of `update()`^[Calibrating models from `update()` is *extremely* time-consuming.].
...
For this example we will use the result from the assessment of a new total hip replacement previously described in `vignette("d-non-homogeneous", "heRomod")`.
